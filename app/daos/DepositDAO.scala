    ...
    filter.map(f => {
      query.filter(table => {
        List(
          ...
          f.crew.map(ids => table._5.filter(_.crew_id === ids.toString()).isDefined),
          f.crewname.map(crews => crews.map(c => table._5.filter(_.name like c).isDefined).reduceLeft(_ || _)),
